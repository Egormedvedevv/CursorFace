...
const tab = document.querySelector(".character-switcher .tab");
let menuOpen = false;

if (tab && characterSwitcher) {
  tab.addEventListener("click", () => {
    menuOpen = !menuOpen;
    if (menuOpen) {
      characterSwitcher.classList.add("touched");
    } else {
      characterSwitcher.classList.remove("touched");
    }
  });
}
...
function updateFaceTarget(x, y) {
  const centerX = window.innerWidth / 2;
  const centerY = window.innerHeight / 2;

  faceTargetX = (x - centerX) * 0.02;
  faceTargetY = (y - centerY) * 0.02;
  faceTargetRotate = (x - centerX) * 0.01;
}

document.addEventListener("mousemove", (e) => {
  updateFaceTarget(e.clientX, e.clientY);
});
...
document.addEventListener("touchmove", (e) => {
  ...
  const touch = e.touches[0];
  if (touch) {
    updateFaceTarget(touch.clientX, touch.clientY);
  }
});
